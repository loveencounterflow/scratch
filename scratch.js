...
(function() {
  ...
  unicode_data = function() {
    var UCS, categories, cid, cid_hex, is_graphic, l, results;
    ...
    is_graphic = function(cid) {
      var category, ref;
      category = categories[cid];
      if (category == null) {
        throw new Error("unable to get category for CID " + (rpr(cid)));
      }
      if (category === 'Zs') {
        return true;
      }
      return (ref = category[0]) === 'L' || ref === 'M' || ref === 'N' || ref === 'P' || ref === 'S';
    };
    results = [];
    for (cid = l = 0x0000; l <= 48; cid = ++l) {
      cid_hex = cid.toString(16);
      results.push(help(cid_hex, categories[cid], CND.truth(is_graphic(cid))));
    }
    return results;
  };

  get_is_cjk_method = function() {
    var ITREE, first_cid, l, last_cid, len, range_name, ref, rsg, tree, unicode_blocks;
    unicode_blocks = [['CJK Radicals Supplement', 'u-cjk-rad2', 0x2e80, 0x2eff], ['Kangxi Radicals', 'u-cjk-rad1', 0x2f00, 0x2fdf], ['CJK Strokes', 'u-cjk-strk', 0x31c0, 0x31ef], ['CJK Unified Ideographs Extension A', 'u-cjk-xa', 0x3400, 0x4dbf], ['CJK Unified Ideographs', 'u-cjk', 0x4e00, 0x9fff], ['CJK Unified Ideographs Extension B', 'u-cjk-xb', 0x20000, 0x2a6df], ['CJK Unified Ideographs Extension C', 'u-cjk-xc', 0x2a700, 0x2b73f], ['CJK Unified Ideographs Extension D', 'u-cjk-xd', 0x2b740, 0x2b81f], ['CJK Unified Ideographs Extension E', 'u-cjk-xe', 0x2b820, 0x2ceaf]];
    ITREE = CND.INTERVALTREE;
    tree = ITREE.new_tree();
    for (l = 0, len = unicode_blocks.length; l < len; l++) {
      ref = unicode_blocks[l], range_name = ref[0], rsg = ref[1], first_cid = ref[2], last_cid = ref[3];
      ITREE.add_interval(tree, [first_cid, last_cid, rsg]);
    }
    return function(cid) {
      return (ITREE.find(tree, cid)).length > 0;
    };
  };

  unicode_data_count = function() {
    var TABLE, category, cid, cid_count, cjk_cid_count, is_cjk, l, len, len1, len2, m, o, ref, ref1, results, table_data, table_settings, version, versions, ƒ;
    ...
    versions = ['unicode-2.0.14', 'unicode-2.1.2', 'unicode-3.0.0', 'unicode-3.1.0', 'unicode-3.2.0', 'unicode-4.0.0', 'unicode-4.1.0', 'unicode-5.0.0', 'unicode-5.1.0', 'unicode-5.2.0', 'unicode-6.0.0', 'unicode-6.1.0', 'unicode-6.2.0', 'unicode-6.3.0', 'unicode-7.0.0', 'unicode-8.0.0'];
    results = [];
    for (l = 0, len = versions.length; l < len; l++) {
      version = versions[l];
      cid_count = 0;
      cjk_cid_count = 0;
      ref = ['L', 'M', 'N', 'P', 'S', 'Zs'];
      for (m = 0, len1 = ref.length; m < len1; m++) {
        category = ref[m];
        ref1 = require(version + "/categories/" + category + "/code-points");
      ...
      results.push(echo(version + "\t" + cid_count + "\t" + cjk_cid_count));
    }
    return results;
  };
  ...
  mkts_backslash_escaping = function() {
    var DIFF, cloaked_text, escape_for_re, metaesc_mc_bs, metaesc_mc_macro, metaesc_mc_master, metaesc_mc_stop, metaesc_mce_bs, metaesc_mce_macro, metaesc_mce_master, metaesc_mce_stop, metaesc_mcp_backslash, metaesc_mcp_bs, metaesc_mcp_macro, metaesc_mcp_master, metaesc_mcp_stop, metaesc_oc_backslash, metaesc_oce_backslash, metaesc_rm_backslash, metaesc_tsc_bs, metaesc_tsc_macro, metaesc_tsc_master, metaesc_tsc_stop, metaesc_tsp_backslash, metaesc_tsp_bs, metaesc_tsp_macro, metaesc_tsp_master, metaesc_tsp_stop, text, uncloaked_text;
    escape_for_re = function(text) {
    ...
    metaesc_mcp_master = RegExp("" + metaesc_mce_master, "g");
    metaesc_mcp_bs = RegExp("" + metaesc_mce_bs, "g");
    metaesc_mcp_macro = RegExp("" + metaesc_mce_macro, "g");
    metaesc_mcp_stop = RegExp("" + metaesc_mce_stop, "g");
    ...
    metaesc_tsp_master = RegExp("" + metaesc_tsc_master, "g");
    metaesc_tsp_bs = RegExp("" + metaesc_tsc_bs, "g");
    metaesc_tsp_macro = RegExp("" + metaesc_tsc_macro, "g");
    metaesc_tsp_stop = RegExp("" + metaesc_tsc_stop, "g");
    ...
    metaesc_mcp_backslash = RegExp(metaesc_oce_backslash + "((?:[\\ud800-\\udbff][\\udc00-\\udfff])|.)", "g");
    metaesc_tsp_backslash = RegExp(metaesc_mc_bs + "([0-9a-f]+)" + metaesc_mc_stop, "g");

    /* `rm`: 'remove' */
    metaesc_rm_backslash = RegExp(metaesc_oce_backslash + "(.)", "g");
    this.escape.cloak_escape_chrs = (function(_this) {
    ...
    this.escape.uncloak_escape_chrs = (function(_this) {
      return function(S, text) {
        var R;
        R = text;
    ...
    this.escape.cloak_backslashed_chrs = (function(_this) {
      return function(S, text) {
        var R;
        R = text;
        R = R.replace(metaesc_mcp_backslash, function(_, $1) {
          var cid;
    ...
    this.escape.uncloak_backslashed_chrs = (function(_this) {
      return function(S, text) {
        var R;
        R = text;
        R = R.replace(metaesc_tsp_backslash, function(_, $1) {
          var chr;
    ...
    this.escape.remove_backslashes = (function(_this) {
      return function(S, text) {
    ...
    log('2', CND.rainbow((cloaked_text = this.escape.cloak_escape_chrs(null, cloaked_text))));
    log('3', CND.rainbow((cloaked_text = this.escape.cloak_backslashed_chrs(null, cloaked_text))));
    uncloaked_text = cloaked_text;
    log('4', CND.rainbow((uncloaked_text = this.escape.uncloak_backslashed_chrs(null, uncloaked_text))));
    log('5', CND.rainbow((uncloaked_text = this.escape.uncloak_escape_chrs(null, uncloaked_text))));
    if (uncloaked_text !== text) {
      log(DIFF.colorize(text, uncloaked_text));
    }
    return log(CND.steel('########################################################################'));
  };

  mkts_illegal_patterns = function() {
    var idx, is_plain, l, len, len1, m, pattern, raw_stretch, ref, ref1, results, stretch, stretches, text;
    this.illegal_patterns = [/(^|[^\\])(<<|>>)/g];
    text = "some << unlicensed >> stuff here.\n\nsome \\<< licensed \\>> stuff here.";
    ...
    for (l = 0, len = ref.length; l < len; l++) {
      ...
      for (idx = m = 0, len1 = ref1.length; m < len1; idx = ++m) {
      ...
      results.push((function() {
        var len2, o, results1;
        results1 = [];
        for (o = 0, len2 = stretches.length; o < len2; o++) {
          stretch = stretches[o];
          is_plain = !is_plain;
          results1.push(info('©54249', is_plain, rpr(stretch)));
        }
        return results1;
      })());
    }
  ...
  interactive_cli = function() {
    var dialogue, filenames, inquirer, onComplete, onEachAnswer, onError, prompt;
    ...
    prompt = inquirer.prompt(dialogue, function(answers) {
      return debug('©ΞΕΡΒΧ', answers);
    });
    onEachAnswer = function() {
      var P;
      P = 1 <= arguments.length ? slice.call(arguments, 0) : [];
      return help('onEachAnswer', P);
    };
    onError = function() {
      var P;
      P = 1 <= arguments.length ? slice.call(arguments, 0) : [];
      return warn('onError', P);
    };
    onComplete = function() {
      var P;
      P = 1 <= arguments.length ? slice.call(arguments, 0) : [];
      return urge('onComplete', P);
    };
    prompt.process.subscribe(onEachAnswer, onError, onComplete);
    return CND.dir(prompt);
  };
  ...
  HOLLERITH_select = function() {
    ...
    HOLLERITH.prune = function(me, prefix, filter, handler) {
      var db_substrate, end_, input, query;
      end_ = null;
      ...
      })(this))).pipe(D.$count((function(_this) {
        return function(count) {
          return help("intersection: " + count);
        };
    ...
    return step((function(_this) {
      return function*(resume) {
        ...
        tasks.push(function(done) {
          return search(source_db, target_db, ['pos', 'guide/has/uchr', '井'], done);
        });
        tasks.push(function(done) {
          return search(source_db, target_db, ['pos', 'guide/has/uchr', '鳥'], done);
        });
